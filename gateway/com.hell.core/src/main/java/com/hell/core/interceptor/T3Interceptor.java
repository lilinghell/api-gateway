...
@Component
public class T3Interceptor implements BaseHandlerInterceptor, ApplicationContextAware {
    private final Logger log = LoggerFactory.getLogger(this.getClass());
    private ApplicationContext applicationContext;

    @Getter
    @Setter
    @Value("${app.t3.commands.pre:}")
    private LinkedHashSet<String> commandsPre = new LinkedHashSet<String>();

    @Getter
    @Setter
    @Value("${app.t3.commands.post:}")
    private LinkedHashSet<String> commandsPost = new LinkedHashSet<String>();

    @Getter
    @Setter
    @Value("${app.t3.commands.after:}")
    private LinkedHashSet<String> commandsAfter = new LinkedHashSet<String>();

    @Getter
    @Setter
    @Value("${app.t3.path-patterns:/api/t3/**}")
    private List<String> pathPatterns = new ArrayList<>();

    @Getter
    @Setter
    @Value("${app.t3.exclude-path-patterns:}")
