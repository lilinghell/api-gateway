<template>
  <div class="q-pa-md">
        ...
        <div class="q-pa-md">
          ...
          <div>
                ...
                <template v-slot:default-header="prop">
                  ...
                  <div class="text-right">
                    <q-icon v-show="
                        ...
                        prop.node.seq != -1 &&
                        prop.node.seq != -2
                      " style="padding-left: 10px" color="green" size="xs" @click="addSubTestGroup(prop.node)" flat name="add_box" />
                    <q-icon v-show="
                        prop.node.children.length === 0 &&
                        prop.node.seq != -1 &&
                        prop.node.seq != -2
                      " style="padding-left: 10px" color="red" size="xs" @click="delConfirmTestGroup(prop.node)" flat name="delete" />
                  </div>
                </template>
              </q-tree>
            </div>
            <div style="padding-top: 40px">
              <p>
                点击<a class="aClass text-h5" @click="addTestGroupOpen">添加</a>一级集合
              </p>
            </div>
          </div>
        </div>
      ...
      <template v-slot:after>
        <div class="q-pa-md">
          <div v-if="node.id === undefined" style="padding-top: 20%" class="row q-col-gutter-sm">
            ...
            <span style="color: grey" class="text-h6">选中集合可查看测试用例，以及进行修改</span>
          </div>
          <div v-else>
            <div class="row">
              <div style="width: 30%">
                <q-input :readonly="node.seq === -1 || node.seq === -2" @keyup.enter="editTestGroup()" @focus="editTestGroupMode()" @blur="editTestGroupMode()" :borderless="!editable" maxlength="30" dense v-model="node.name" type="input" />
              </div>
              <div style="width: 70%" class="text-right">
                <q-btn flat color="primary" @click="testCaseRunAll" icon="playlist_play" label="运行" />
                <q-btn flat @click="editTestGroup" color="primary" icon="save" label="保存" />
                <q-btn color="primary" flat dense icon="more_horiz">
                  <q-menu transition-show="jump-down" transition-hide="jump-up">
                    <q-list style="width: 230px">
                      <q-item-label header>测试报告</q-item-label>
                      <q-item clickable @click="showTestPlanReportAll" v-close-popup>
            ...
            <q-separator />
            <div class="row">
              <q-tabs style="width: 30%" class="text-left" v-model="apiTestGroupSelectType" narrow-indicator dense align="justify">
                <q-tab name="overView" class="text-primary text-left" label="概述" />
                <q-tab name="cases" class="text-primary text-left" label="用例列表" />
              </q-tabs>
              <q-tab-panels @transition="initEcharShow" keep-alive style="width: 100%" v-model="apiTestGroupSelectType" animated swipeable vertical transition-prev="jump-up" transition-next="jump-up">
                ...
                <q-tab-panel name="cases">
                  <div class="q-mb-md">
                    <q-table flat :data="testCasesList" :columns="columns" :filter="testCasesfilter" row-key="index" :pagination.sync="pagination" hide-pagination>
                      <template v-slot:top-left>
                        <q-input borderless dense debounce="300" v-model="testCasesfilter" placeholder="Search">
                          <template v-slot:append>
                      ...
                      <template v-slot:top-right>
                        <q-btn color="primary" size="12px" unelevated class="table-head-btn" @click="addTestCasesModelOpen">
                          新增
                      ...
                      <template v-slot:body-cell-status="props">
                        <q-td :class="props.col.__tdClass" class="q-gutter-x-sm">
                          <span class="text-teal" v-if="props.row.status === '0'">执行成功</span>
                          <span class="text-red" v-if="props.row.status === '1'">执行失败</span>
                          <span class="text-primary" v-if="props.row.status === '2'">待执行</span>
                          <span class="text-orange" v-if="props.row.status === '3'">执行中</span>
                          <span class="text-teal" v-if="props.row.status === '4'">验证成功</span>
                          <span class="text-red" v-if="props.row.status === '5'">验证失败</span>
                        </q-td>
                      </template>
                      <template v-slot:body-cell-operation="props">
                        <q-td :class="props.col.__tdClass" class="q-gutter-x-sm">
                          <q-btn @click="showTestPlanReport(props.row)" :disabled="
                              props.row.status === '2' ||
                              props.row.status === '3'
                            " flat color="primary" label="详细报告" />
                          <q-btn color="primary" flat round dense icon="more_horiz">
                            <q-menu transition-show="jump-down" transition-hide="jump-up">
                              <q-list style="min-width: 100px">
                                <q-item @click="testCaseRun(props.row)" clickable v-close-popup>
                                  <q-item-section>运行</q-item-section>
                                </q-item>
                                <q-separator />
                                <q-item @click="copyTestCases(props.row)" clickable v-close-popup>
                                  <q-item-section>复制</q-item-section>
                                </q-item>
                                <q-item @click="mgmtTestCases(props.row)" clickable v-close-popup>
                                  <q-item-section>编辑</q-item-section>
                                </q-item>
                                <q-separator />
                                <q-item @click="delTestCases(props.row)" clickable v-close-popup>
    ...
    <q-dialog v-model="handAddTestGroupMode" persistent>
      <q-card style="min-width: 450px">
        ...
        <q-form @submit="addTestGroup" ref="addTestGroupForm">
          <q-card-section style="width: 430px" class="text-right">
            <q-input filled autofocus v-model="addTestGroupModel.name" type="text" :rules="[(val) => (val && val.length > 0) || '']">
              <template v-slot:before>
                <span style="font-size: 15px; width: 80px">集合名称：</span>
              </template>
            </q-input>
            <q-input filled v-model="addTestGroupModel.info" type="textarea">
              <template v-slot:before>
    ...
    <q-dialog v-model="confirm" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="确定" color="primary" @click="delTestGroup()" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    <q-dialog v-model="delConfirm" persistent>
      <q-card>
        ...
        <q-card-actions align="right">
          <q-btn label="删除" @click="delTest" color="primary" v-close-popup />
          <q-btn flat label="取消" color="primary" v-close-popup />
        </q-card-actions>
      </q-card>
    </q-dialog>
        ...
        <q-card>
          <div class="q-pa-md row q-col-gutter-sm">
            <div class="col-12 col-sm-5">
              ...
              <div class="row">
                <q-input style="width: 60%" v-model="addTestCaseModel.name" label="用例名称" :rules="[(val) => (val && val.length > 0) || '']" />
                <q-radio v-if="node.seq != -2" dense style="padding-left: 15px" v-model="addTestCaseModel.caseType" val="0" label="正例" color="orange" />
                <q-radio v-if="node.seq != -2" dense style="padding-left: 15px" v-model="addTestCaseModel.caseType" val="1" label="反例" color="red" />
                <q-radio v-if="node.seq === -2" dense style="padding-left: 15px" v-model="addTestCaseModel.caseType" val="2" label="基础用例" color="red" />
              </div>
              <q-select :display-value="
                  formatDicDisplay(apiOptions, addTestCaseModel.apiPath)
                " :readonly="mgmtFlag" @input="qryShili" label="UAT环境的接口路径" filled v-model="addTestCaseModel.apiPath" use-input hide-selected fill-input input-debounce="0" :options="apiOptions" @filter="apiFilter" :rules="[(val) => val || '']" style="width: 95%; padding-bottom: 32px">
              ...
              <q-input style="width: 95%" label="描述" filled v-model="addTestCaseModel.caseDesc" type="textarea" />
              <pre><span v-if="showResponseDemo" style="color:grey">接口应答示例：{{ JSON.stringify(responseDemo, null, 2)  }}</span></pre>
              <div v-if="showParameter">
                <div>所有常量</div>
                <q-scroll-area :thumb-style="thumbStyle" :bar-style="barStyle" style="height: 400px; max-width: 100%">
                  <div class="q-col-gutter-sm">
                    <div v-for="(para, index) in parameterList" :key="'para_' + index">
                      <div class="row">
            ...
            <div class="col-12 col-sm-6 q-gutter-sm">
              ...
              <q-input @click.stop="showok" @keydown.native="listen($event)" name="testParameterName" rows="14" dense filled v-model="addTestCaseModel.parameter" type="textarea" />

              <div class="text-h6 text-primary">»编写断言脚本</div>
              <p>格式：$res.body.xx == xx</p>
              <q-input @input="assertScriptFun" dense filled v-model="addTestCaseModel.assertScript" type="textarea" />
            </div>
          </div>
        </q-card>
        <q-footer class="bg-white text-white">
          <q-toolbar inset>
            <q-toolbar-title class="text-right">
              <q-btn @click="saveParaSet()" label="保存" color="primary" />
            </q-toolbar-title>
          </q-toolbar>
        </q-footer>
      </q-layout>
...
import { positiveNotify, warningNotify, formatDicDisplay, html2pdf } from '@/utils/utils'
...
export default {
  ...
  methods: {
    ...
    async qryShili(api) {
      ...
      apiTables.forEach((apiTable) => {
        apiTable.forEach((api) => {
          if (!this.mgmtFlag || document.getElementsByName('testParameterName')[0].value === '') {
            let reqStr = ''
            api.requestDemo.forEach((demo) => {
              reqStr = reqStr + demo + '\n'
            })
            if (Object.keys(api.requestObjectDemo).length !== 0) {
              reqStr = reqStr + ' -d ' + "'" + JSON.stringify(api.requestObjectDemo, null, 2) + "'"
            }
            document.getElementsByName('testParameterName')[0].value = reqStr
            document.getElementsByName('testParameterName')[0].dispatchEvent(new Event('input'))
          }
    ...
    mgmtTestCases(row) {
      ...
      let fi = this.apiList.filter((api) => api.url.toString() === row.apiPath.toString())
      this.qryShili(fi[0])
    },
    ...
    async delTestGroup() {
      ...
      this.testGroupList = this.testGroupList.filter((testGroup) => testGroup.seq != this.node.seq)
      this.createTestGroupTree()
      positiveNotify('删除成功!')
      this.confirm = false
      this.node = []
